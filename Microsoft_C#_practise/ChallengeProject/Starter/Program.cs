...
foreach (string name in studentNames)
{
    ...
    */
    ...
    // calculating Exam Score (first five scores divided by amount of required assigments - 5)
    foreach (int points in studentScores)
    {
        gradedAssignments += 1;
    ...
    // reset gradedAssigmnts to 0
    gradedAssignments = 0;

    // calcualting overall grade inclduing all students scores
    ...
    foreach (int credit in studentScores)
    {
        gradedAssignments += 1;
    ...
    Console.WriteLine($"{currentStudent}\t\t{examScore}\t\t{overallGrade}\t{currentStudentLetterGrade}\t{extraCredit} ({extraPoints} pts)");
}
